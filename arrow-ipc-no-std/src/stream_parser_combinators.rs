...
pub enum ArrowMessageParseError {
    ...
    Flatbuffers,
    /// Length of metadata or body out of usize bounds.
    #[snafu(display("length of metadata or body out of usize bounds"))]
...
fn message_parser<'a>(input: &'a [u8]) -> Result<(&'a [u8], Message<'a>), ArrowMessageParseError> {
    ...
    let message_length = usize::try_from(metadata_len)
        .and_then(|metadata_len| {
            Ok(metadata_len.saturating_add(usize::try_from(message.bodyLength())?))
        })
        .map_err(|_| ArrowMessageParseError::LengthOutOfUsizeBounds)?;
...
mod tests {
    ...
    proptest! {
        ...
        fn continuation_parser_does_not_panic(bytes in proptest::collection::vec(any::<u8>(), 0..16)) {
            let no_panic = continuation_parser(&bytes);

            assert!(no_panic.is_ok() || no_panic.is_err());
        }

        #[test]
        fn le_u32_parser_does_not_panic(bytes in proptest::collection::vec(any::<u8>(), 0..16)) {
            let no_panic = le_u32_parser(&bytes);
        ...
        fn schema_parser_does_not_panic(
            ...
            let no_panic = schema_parser(&bytes);

            assert!(no_panic.is_err() || no_panic.is_ok());
        }
        ...
        fn record_batch_parser_does_not_panic(
            ...
            let no_panic = record_batch_parser(&bytes);
        ...
        fn end_of_stream_parser_does_not_panic(bytes in proptest::collection::vec(any::<u8>(), 0..16)) {
            let no_panic = end_of_stream_parser(&bytes);
        ...
        fn single_batch_stream_parser_does_not_panic(
            ...
            let no_panic = single_batch_stream_parser(&bytes);
