        ...
        + frame_system::Config<AccountData = pallet_balances::AccountData<u128>>,
{
    let pre_hashed_leaves =
        encode_prefix_leaves::<CommitmentMapPrefixFoliate<T>, _>(commitment_prefix_iter)?
            .into_iter()
            .chain(encode_prefix_leaves::<AccountPrefixFoliate<T>, _>(
                account_prefix_iter,
            )?)
...
mod tests {
    ...
    use frame_system::{AccountInfo, _GeneratedPrefixForStorageAccount};
    ...
    fn valid_attestation_tree_and_items() -> (
        MerkleTree,
        AccountId32,
        AccountInfo<Nonce, AccountData<u128>>,
        TableIdentifier,
        CommitmentScheme,
        ...
        let account_key_bytes = _GeneratedPrefixForStorageAccount::<Runtime>::prefix_hash()
            .into_iter()
        ...
        let account_data = AccountData::<u128> {
            free: 257,
            reserved: 1024,
            frozen: 514,
        ...
        let account_info_bytes = account_info.encode();

        let account_iter = [(account_key_bytes, account_info_bytes)];
        ...
        let attestation_tree =
            attestation_tree_from_prefixes::<_, _, Runtime>(commitments_iter, account_iter)
                .unwrap();
    ...
    fn we_can_prove_leaves_in_attestation_tree() {
        ...
        let account_leaf = keccak256(
            &keccak256(&hex::encode(encode_key_value_leaf::<
                AccountPrefixFoliate<Runtime>,
            >(
                (account_id.clone(),), account_info.clone()
            )))
            .unwrap(),
        )
        .unwrap();
        ...
        let proof_of_account = prove_leaf_pair::<AccountPrefixFoliate<Runtime>>(
            &attestation_tree,
            (account_id,),
            account_info,
        )
        ...
        dbg!(&proof_of_account);

        let proven_root_hash = std::iter::once(account_leaf)
            .chain(proof_of_account.into_iter().map(|h| h[2..].to_string()))
    ...
    fn we_cannot_create_attestation_tree_from_invalid_data() {
        let result = attestation_tree_from_prefixes::<_, _, Runtime>([], [(vec![0], vec![1])]);
    ...
    fn we_cannot_prove_leaf_that_does_not_exist() {
        let (attestation_tree, _, _, table_identifier, _, table_commitment_bytes) =
            valid_attestation_tree_and_items();
