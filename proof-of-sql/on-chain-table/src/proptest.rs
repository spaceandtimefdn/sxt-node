...
/// A table schema definition using proof-of-sql identifiers/types.
#[derive(Clone, Debug, PartialEq, Eq)]
pub struct ProofOfSqlSchema(Vec<(Ident, ColumnType)>);

/// Cannot create [`ProofOfSqlSchema`] with 0 columns.
#[derive(Debug, Snafu)]
#[snafu(display("cannot create proof-of-sql schema with 0 columns"))]
pub struct NoColumns;

impl ProofOfSqlSchema {
    /// Try to construct a new [`ProofOfSqlSchema`].
    ...
    /// Return the internal collection as a slice.
    pub fn as_slice(&self) -> &[(Ident, ColumnType)] {
        &self.0
    }

    /// Consume `self` and return the internal collection.
...
prop_compose! {
    /// Strategy for constructing sqlparser `Ident`s.
...
prop_compose! {
    /// Strategy for producing `ColumnType`s of variant `ColumnType::Decimal75`.
...
/// Strategy for producing `ColumnType`s that are supported by the chain.
pub fn supported_column_type() -> impl Strategy<Value = ColumnType> {
    prop_oneof![
...
/// Strategy for producing valid [`ProofOfSqlSchema`]s.
///
/// Note: will always have at least 1 column, even if `num_columns` is `0..=0`.
...
/// Strategy for producing an arrow i256 with no more than `max_num_digits` digits.
pub fn i256_with_max_num_digits(max_num_digits: u8) -> impl Strategy<Value = i256> {
...
prop_compose! {
    /// Strategy for producing any arrow i256.
    pub fn i256()
...
/// Strategy for producing [`OnChainColumn`]s with the variant [`OnChainColumn::Decimal75`].
///
/// Note: depending on the element strategy `E`, values that are "out of scalar bounds" may be
/// produced, and attempts to commit will error.
/// This can be useful if you're testing that sad path.
/// Use [`i256_with_max_num_digits(scale)`] if you want values within bounds.
...
/// Strategy for producing [`OnChainColumn`]s of the given column type and length, which can
/// themselves be strategies.
pub fn on_chain_column<CT, NR>(
    column_type: CT,
    num_rows: NR,
) -> impl Strategy<Value = OnChainColumn>
where
...
/// Strategy for producing an [`OnChainTable`] with the given schema and length, which can
/// themselves be strategies.
