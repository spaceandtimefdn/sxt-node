...
//!   Reads `frame_system::Events`, extracts relevant variants, writes a
...
pub mod pallet {
    ...
    #[pallet::config]
    pub trait Config: frame_system::Config + pallet_tables::Config {
    ...
    pub enum Event<T: Config> {
        ...
        BlockForwarded { block_number: u64 },
        /// The offchain indexer encountered an error.
        ForwardingError { block_number: u64 },
    }
    ...
    impl<T: Config> Pallet<T> {
        ...
        fn try_extract_indexing_event(
            ...
            let Ok(data) = <sp_runtime::BoundedVec<
                u8,
                frame_support::traits::ConstU32<{ sxt_core::indexing::DATA_MAX_LEN }>,
            >>::decode(&mut input) else {
