...
pub fn handle_message<T: Config>(sender: T::AccountId, message_bytes: Vec<u8>) -> DispatchResult {
    // Parse the message
    let raw_hex = from_utf8(&message_bytes).map_err(|e| Error::<T>::InvalidMessageFormat)?;

    let raw_bytes = Vec::from_hex(raw_hex.trim_start_matches("0x")).expect("valid hex string");

    let keys = T::Keys::decode(&mut &raw_bytes[..]).expect("should decode into SessionKeys");
