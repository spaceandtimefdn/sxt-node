...
//!
//! Mirrors `pallet_indexing`'s mock because our `Config<I>: pallet_indexing::Config<I>`
//! supertrait pulls in the same transitive dependency chain (system_tables
//! → session/staking/balances/zkpay/timestamp). The mock exists purely to
//! satisfy those trait bounds at compile time — no test actually exercises
//! the producer side that consumes events from these pallets.
...
impl pallet_prover_db_indexer::pallet::Config<Api> for Test {
    type RuntimeEvent = RuntimeEvent;
}
