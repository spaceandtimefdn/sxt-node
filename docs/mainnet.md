...
docker run -it --rm \
  ...
  -c '
    ...
    if [ ! -d /data/chains ]; then
      cd /data
      curl -O "https://blocks.testnet.sxt.network/snapshots/2026-01-26/sxt-mainnet.tar.gz"
      curl -O "https://blocks.testnet.sxt.network/snapshots/2026-01-26/sxt-mainnet.sha1"
